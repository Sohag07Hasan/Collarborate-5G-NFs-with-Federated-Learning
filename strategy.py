import flwr as fl
from config import FRACTION_FIT, FRACTION_EVAL, NUM_CLIENTS
...
def create_strategy(fit_config_fn: Callable, weighted_average_fn: Callable, evaluate_fn: Callable, fit_metrics_aggregation_fn: Callable):
    strategy = fl.server.strategy.FedAvg(
        ...
        min_fit_clients=NUM_CLIENTS,  # This ensures that at least NUM_CLIENTS must be available and must successfully complete training in each round. If fewer than this number of clients are available or successfully complete training, the round will fail
        min_evaluate_clients=NUM_CLIENTS,  # Minimum number of clients for evaluation
